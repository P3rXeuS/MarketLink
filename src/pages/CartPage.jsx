...
const CartPage = () => {
  ...
  const handleIncrease = (id) => {
    ...
    if (item && product && item.quantity < product.quantity) {
      ...
      // console.log("Increased quantity for item:", updatedCartItems);
    } else {
  ...
  const handleDecrease = (id) => {
    ...
    // console.log("Decreased quantity for item:", updatedCartItems);
  };
  ...
  const handleDelete = (id) => {
    ...
    // console.log("Deleted item from cart:", updatedCartItems);
  };
  ...
  const handleBuy = () => {
    ...
    const updatedProducts = products.map((prod) => {
      ...
      if (cartItem && selectedItems.has(prod.id)) {
        if (prod.quantity < cartItem.quantity) {
          toast.error(`Insufficient stock for: ${cartItem.title}`);
          return prod;
        }
    ...
    if (purchaseSuccess) {
      ...
      // console.log("Purchase successful, updated cart and products:", {
      //   remainingCartItems,
      //   updatedProducts,
      // });
    }
    ...
    return (
      <div className="bg-gray-100 min-h-screen py-8">
        ...
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6">
          ...
          <CartSummary total={0} handleBuy={handleBuy} selectedItems={new Set()} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8">
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6">
            ...
            <div key={item.id} className="flex items-start justify-between py-4 border-b gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-base font-bold text-gray-800">${item.price.toLocaleString()}</p>
                  <p className="text-base font-bold text-gray-800">
                    Available stock: {products.find((prod) => prod.id === item.id)?.quantity ?? "Not available"}
                  </p>
                </div>
              </div>
        ...
        <CartSummary total={total} handleBuy={handleBuy} selectedItems={selectedItems} />
      </div>
    </div>
  );
};
